...
namespace lightstep {
class LightStepTracerImpl2 final
    : public LightStepTracer,
      public std::enable_shared_from_this<LightStepTracerImpl2> {
 public:
  LightStepTracerImpl2(const PropagationOptions& propagation_options,
                      std::unique_ptr<Recorder>&& recorder) noexcept;

  LightStepTracerImpl2(std::shared_ptr<Logger> logger,
